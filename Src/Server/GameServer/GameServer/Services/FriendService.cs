...
namespace GameServer.Services
{
    public class FriendService : Singleton<FriendService>
    {
        ...
        void OnFriendRemove(NetConnection<NetSession> sender,FriendRemoveRequest request)
        {
            ...
            if (character.FriendManager.RemoveFriendByID(request.Id))
            {
                ...
                {//好友在线
                    ...
                    friend.Session.Response.friendRemove = new FriendRemoveResponse();
                    friend.Session.Response.friendRemove.Result = Result.Success;
                    friend.SendResponse();
                }
                else
                {//不在线
        ...
        {
            var removeItem = DBService.Instance.Entities.CharacterFriends.FirstOrDefault(ValueType => ValueType.CharacterID == charId && ValueType.FriendID == friendId);
            if(removeItem!=null)
            {
