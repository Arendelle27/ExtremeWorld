...
namespace Services
{
    ...
    {
        internal Action<bool> OnGuildCreateResult;
        internal Action<List<NGuildInfo>> OnGuildListResult;
        internal Action OnGuildUpdate;
        ...
        void OnGuild(object sender,GuildResponse message)
        {
        ...
        private void OnGuildLeave(object sender, GuildLeaveResponse message)
        {
            if (message.Result == Result.Success)
            {
                ...
                MessageBox.Show("离开队伍成功", "公会");
            }
            else
                MessageBox.Show("离开队伍失败", "公会",MessageBoxType.Error);
        }
        ...
        {
            Debug.Log("SendGuildJoinApplyResponse");
            NetMessage message = new NetMessage();
            message.Request = new NetMessageRequest();
            message.Request.guildJoinRes = new GuildJoinResponse();
            message.Request.guildJoinRes.Result = Result.Success;
