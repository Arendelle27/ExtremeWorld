...
namespace Services
{
    public class QuestService:Singleton<QuestService>,IDisposable
    {
        ...
        {
            Debug.LogFormat("OnQuestAcceptResponse:{0},ERR:{1}",message.Result,message.Errormsg);
        ...
        {
            Debug.LogFormat("OnQuestSubmitResponse:{0},ERR:{1}", message.Result, message.Errormsg);
            if(message.Result == Result.Success)
            {
